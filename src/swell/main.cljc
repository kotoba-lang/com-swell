(ns swell.main
  "Kotodama WASM entrypoint for the Swell clean-room actor (L4) — Clojure port.
  ...
  facts (`swell.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"swell.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "swell-compat" :tier tier :entities entities} 200])
